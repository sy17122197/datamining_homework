...
def build_RNN_model(vocab, config, train=True):
    model = RNNModel(vocab.vocab_size, config)
    if train:
        model.train()  
        #在训练模型时会在前面加上train();
    else:
...
def train_RNN_model(model, train_data, valid_data, padding_idx, config):
    # Build optimizer.
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        #每个eopch训练样本调用一次验证集。
        valid_loss = valid_RNN_model(model, valid_data, criterion, config)
        # 处理
...
def valid_RNN_model(model,  valid_data, criterion, config):
    # Build optimizer.
    # params = [p for k, p in model.named_parameters(
    # ) if p.requires_grad and "embed" not in k]
    model.eval()   
...
def main():
    ...
    # 构建textcnn模型
    model = build_RNN_model(vocab, config, train=True)

    print(model)

    # Do training.
    padding_idx = vocab.stoi[PAD]
    train_RNN_model(model, train_data,
